...
import { _ as _export_sfc, F as FlashMessages } from "./FlashMessages-BlPv0OK4.js";
...
const _sfc_main = {
  ...
  methods: {
    ...
    handleForwardingRequest(status) {
      ...
      axios.post(this.route("tickets.forwarding.handle", this.forwarding_request.id), { status }).then((response) => {
...
function _sfc_ssrRender(_ctx, _push, _parent, _attrs, $props, $setup, $data, $options) {
  ...
  _push(`<div class="flex flex-wrap"><div class="max-w-full lg:w-3/5"><form class="bg-white rounded-md shadow overflow-hidden mr-2">`);
  _push(ssrRenderComponent(_component_flash_messages, null, null, _parent));
  _push(`<div class="p-8 -mr-6 -mb-8 flex flex-wrap">`);
  ...
  if ($props.auth.user.role.slug !== "customer" && !($props.hidden_fields && $props.hidden_fields.includes("assigned_to"))) {
    _push(ssrRenderComponent(_component_select_edit_input, {
      ...
      editable: $data.user_access.ticket.update && !$props.ticket.closed && $props.auth.user.role.slug === "admin" || $data.is_team_head
    }, null, _parent));
  } else {
    _push(`<!---->`);
  }
  ...
  if (!($props.hidden_fields && $props.hidden_fields.includes("department"))) {
    _push(ssrRenderComponent(_component_select_edit_input, {
      ...
      editable: $data.user_access.ticket.update && !$props.ticket.closed && $props.auth.user.role.slug === "admin" || $data.is_team_head && (!$props.forwarding_request || $props.forwarding_request.status === "approved")
    }, null, _parent));
  } else {
    _push(`<!---->`);
  }
  if ($props.forwarding_request && $props.forwarding_request.status === "rejected") {
    _push(`<div class="pr-6 pb-8 w-full lg:w-1/3 text-red-600"> A forwarding request for this ticket has been rejected. </div>`);
  } else {
    _push(`<!---->`);
  }
  if ($data.is_team_head && $props.forwarding_request && $props.forwarding_request.status === "pending") {
    _push(`<div class="pr-6 pb-8 w-full text-red-600"> A forward request was sent to the admin from your department. Waiting for admin review. </div>`);
  } else {
    _push(`<!---->`);
  }
  ...
  _push(`</div><input type="file" accept=".xlsx,.xls,image/*,.doc, .docx,.ppt, .pptx,.txt,.pdf, .zip" class="hidden" multiple="multiple"><div class="pr-6 pb-8 w-full lg:w-full flex-col"><button type="button" class="btn flex justify-center items-center relative z-10 pb-3 border-0 pl-0">`);
  _push(ssrRenderComponent(_component_icon, {
    name: "file",
  ...
  _push(`</div>`);
  if ($props.auth.user.role.slug === "admin" && $props.forwarding_request && $props.forwarding_request.status === "pending") {
    _push(`<div class="px-8 py-4 bg-yellow-100 border-t border-gray-100 flex items-center"><div class="mr-4">This ticket has a pending forwarding request from ${ssrInterpolate($props.forwarding_request.old_department.name)} to ${ssrInterpolate($props.forwarding_request.new_department.name)}.</div>`);
    _push(ssrRenderComponent(_component_loading_button, {
      loading: $data.form.processing,
      onClick: ($event) => $options.handleForwardingRequest("approved"),
    ...
    _push(ssrRenderComponent(_component_loading_button, {
      loading: $data.form.processing,
      onClick: ($event) => $options.handleForwardingRequest("rejected"),
      class: "btn-red"
    }, {
      default: withCtx((_, _push2, _parent2, _scopeId) => {
        if (_push2) {
  ...
  _push(`<div class="px-8 py-4 bg-gray-50 border-t border-gray-100 flex items-center">`);
  ...
  _push(ssrRenderComponent(_component_QuillEditor, {
    ...
    "onUpdate:content": ($event) => $data.comment = $event,
    contentType: "html"
  }, null, _parent));
