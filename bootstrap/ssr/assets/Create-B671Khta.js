...
function _sfc_ssrRender(_ctx, _push, _parent, _attrs, $props, $setup, $data, $options) {
  ...
  _push(`<div${ssrRenderAttrs(_attrs)} data-v-66c06259><div class="bg-white rounded-md shadow overflow-hidden max-w-full" data-v-66c06259><form data-v-66c06259><div class="p-8 -mr-6 -mb-8 flex flex-wrap" data-v-66c06259>`);
  ...
  if ($data.user_access.ticket.update && $props.auth.user.role.slug !== "customer") {
    _push(ssrRenderComponent(_component_select_input, {
      ...
      default: withCtx((_, _push2, _parent2, _scopeId) => {
        if (_push2) {
          _push2(`<option${ssrRenderAttr("value", null)} data-v-66c06259${_scopeId}></option><!--[-->`);
          ssrRenderList($props.priorities, (s) => {
            _push2(`<option${ssrRenderAttr("value", s.id)} data-v-66c06259${_scopeId}>${ssrInterpolate(s.name)}</option>`);
          });
          _push2(`<!--]-->`);
        } else {
          return [
  ...
  if (!($props.hidden_fields && $props.hidden_fields.includes("department")) && $props.auth.user.role.slug !== "customer") {
    _push(ssrRenderComponent(_component_select_input, {
      ...
      default: withCtx((_, _push2, _parent2, _scopeId) => {
        if (_push2) {
          _push2(`<option${ssrRenderAttr("value", null)} data-v-66c06259${_scopeId}>${ssrInterpolate(_ctx.$t("Select a department"))}</option><!--[-->`);
          ssrRenderList($props.departments, (department) => {
            _push2(`<option${ssrRenderAttr("value", department.id)} data-v-66c06259${_scopeId}>${ssrInterpolate(department.name)}</option>`);
          });
          _push2(`<!--]-->`);
        } else {
          return [
  ...
  if (!($props.hidden_fields && $props.hidden_fields.includes("category")) && $data.form.department_id) {
    _push(ssrRenderComponent(_component_select_input, {
      ...
      default: withCtx((_, _push2, _parent2, _scopeId) => {
        if (_push2) {
          _push2(`<option${ssrRenderAttr("value", null)} data-v-66c06259${_scopeId}>${ssrInterpolate(_ctx.$t("Select a category"))}</option><!--[-->`);
          ssrRenderList($data.categories, (category) => {
            _push2(`<option${ssrRenderAttr("value", category.id)} data-v-66c06259${_scopeId}>${ssrInterpolate(category.name)}</option>`);
          });
          _push2(`<!--]-->`);
        } else {
          return [
  ...
  if ($data.form.category_id) {
    _push(ssrRenderComponent(_component_select_input, {
      ...
      default: withCtx((_, _push2, _parent2, _scopeId) => {
        if (_push2) {
          _push2(`<option${ssrRenderAttr("value", null)} data-v-66c06259${_scopeId}>${ssrInterpolate(_ctx.$t("Select a sub category"))}</option><!--[-->`);
  ...
  ssrRenderList($props.custom_fields, (ticket_field) => {
    _push(`<div class="pr-6 pb-8 w-full lg:w-1/3" data-v-66c06259><div class="flex flex-col w-full" data-v-66c06259><label${ssrRenderAttr(
      "for",
      !["checkbox"].includes(ticket_field.type) ? "ticket_field_" + ticket_field.id : null
    )} class="form-label" data-v-66c06259>${ssrInterpolate(ticket_field.label)} `);
    if (!ticket_field.required) {
      _push(`<small class="pl-2 text-xs" data-v-66c06259>(optional)</small>`);
    } else {
      _push(`<!---->`);
    }
    _push(`</label>`);
    if (["text", "email", "number"].includes(ticket_field.type)) {
      _push(`<input${ssrRenderDynamicModel(ticket_field.type, $data.form.custom_field[ticket_field.name], null)}${ssrRenderAttr("type", ticket_field.type)}${ssrRenderAttr("id", "ticket_field_" + ticket_field.id)}${ssrRenderAttr("placeholder", ticket_field.placeholder)}${ssrIncludeBooleanAttr(ticket_field.required) ? " required" : ""} class="form-input" data-v-66c06259>`);
    } else {
      _push(`<!---->`);
    }
    if (ticket_field.type === "textarea") {
      _push(`<textarea${ssrRenderAttr("id", "ticket_field_" + ticket_field.id)} rows="2" class="form-textarea"${ssrRenderAttr("placeholder", ticket_field.placeholder)} data-v-66c06259>${ssrInterpolate($data.form.custom_field[ticket_field.name])}</textarea>`);
    } else {
      _push(`<!---->`);
    }
    if (ticket_field.hint) {
      _push(`<p class="mt-1 text-sm text-gray-500 dark:text-gray-300" data-v-66c06259>${ssrInterpolate(ticket_field.hint)}</p>`);
    } else {
      _push(`<!---->`);
    }
    _push(`</div></div>`);
  ...
  if ($data.suggestions.faqs.length || $data.suggestions.knowledge_base.length) {
    _push(`<div class="w-full mb-8" data-v-66c06259><div class="bg-gradient-to-r from-indigo-600 to-purple-600 text-white px-6 py-4 rounded-t-xl" data-v-66c06259><div class="flex items-center space-x-3" data-v-66c06259><svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24" data-v-66c06259><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" data-v-66c06259></path></svg><h2 class="text-xl font-semibold" data-v-66c06259> You may also find solutions in these topics </h2></div></div><div class="bg-white border-x border-b border-gray-200 rounded-b-xl shadow-lg" data-v-66c06259><div class="max-h-[350px] overflow-y-auto p-6 space-y-8" data-v-66c06259>`);
    if ($data.suggestions.faqs.length) {
      _push(`<div class="space-y-4" data-v-66c06259><div class="flex items-center space-x-2 pb-3 border-b border-gray-100" data-v-66c06259><svg class="w-5 h-5 text-amber-500" fill="currentColor" viewBox="0 0 20 20" data-v-66c06259><path fill-rule="evenodd" d="M18 10a8 8 0 11-16 0 8 8 0 0116 0zm-8-3a1 1 0 00-.867.5 1 1 0 11-1.731-1A3 3 0 0113 8a3.001 3.001 0 01-2 2.83V11a1 1 0 11-2 0v-1a1 1 0 011-1 1 1 0 100-2zm0 8a1 1 0 100-2 1 1 0 000 2z" clip-rule="evenodd" data-v-66c06259></path></svg><h3 class="text-lg font-bold text-gray-800" data-v-66c06259> Frequently Asked Questions </h3><span class="bg-amber-100 text-amber-800 text-xs font-medium px-2.5 py-0.5 rounded-full" data-v-66c06259>${ssrInterpolate($data.suggestions.faqs.length)}</span></div><div class="flex flex-wrap gap-4" data-v-66c06259><!--[-->`);
      ssrRenderList($data.suggestions.faqs, (faq) => {
        _push(`<div class="group relative bg-gradient-to-r from-amber-50 to-orange-50 hover:from-amber-100 hover:to-orange-100 rounded-lg border border-amber-200 hover:border-amber-300 transition-all duration-200 hover:shadow-md" data-v-66c06259><a${ssrRenderAttr("href", _ctx.route("faq"))} target="_blank" class="block p-4 h-full" data-v-66c06259><div class="flex items-start justify-between" data-v-66c06259><div class="flex-1 min-w-0" data-v-66c06259><h4 class="font-semibold text-gray-900 group-hover:text-amber-700 transition-colors duration-200 mb-2 line-clamp-2" data-v-66c06259>${ssrInterpolate(faq.name)}</h4><p class="text-sm text-gray-600 group-hover:text-gray-700 line-clamp-3" data-v-66c06259>${ssrInterpolate($options.truncate(faq.details, 170))}</p></div><svg class="w-5 h-5 text-amber-400 group-hover:text-amber-600 transition-colors duration-200 ml-3 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24" data-v-66c06259><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 6H6a2 2 0 00-2 2v10a2 2 0 002 2h10a2 2 0 002-2v-4M14 4h6m0 0v6m0-6L10 14" data-v-66c06259></path></svg></div></a></div>`);
      });
      _push(`<!--]--></div></div>`);
    } else {
      _push(`<!---->`);
    ...
    if ($data.suggestions.knowledge_base.length) {
      _push(`<div class="space-y-4" data-v-66c06259><div class="flex items-center space-x-2 pb-3 border-b border-gray-100" data-v-66c06259><svg class="w-5 h-5 text-blue-500" fill="currentColor" viewBox="0 0 20 20" data-v-66c06259><path d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" data-v-66c06259></path></svg><h3 class="text-lg font-bold text-gray-800" data-v-66c06259>Knowledge Base</h3><span class="bg-blue-100 text-blue-800 text-xs font-medium px-2.5 py-0.5 rounded-full" data-v-66c06259>${ssrInterpolate($data.suggestions.knowledge_base.length)}</span></div><div class="flex flex-wrap gap-4" data-v-66c06259><!--[-->`);
      ssrRenderList($data.suggestions.knowledge_base, (kb) => {
        _push(`<div class="group relative bg-gradient-to-r from-blue-50 to-indigo-50 hover:from-blue-100 hover:to-indigo-100 rounded-lg border border-blue-200 hover:border-blue-300 transition-all duration-200 hover:shadow-md" data-v-66c06259><a${ssrRenderAttr("href", _ctx.route("kb.details", kb.id))} target="_blank" class="block p-4 h-full" data-v-66c06259><div class="flex items-start justify-between" data-v-66c06259><div class="flex-1 min-w-0" data-v-66c06259><h4 class="font-semibold text-gray-900 group-hover:text-blue-700 transition-colors duration-200 mb-2 line-clamp-2" data-v-66c06259>${ssrInterpolate(kb.title)}</h4><p class="text-sm text-gray-600 group-hover:text-gray-700 line-clamp-3" data-v-66c06259>${ssrInterpolate($options.truncate(kb.details, 170))}</p></div><svg class="w-5 h-5 text-blue-400 group-hover:text-blue-600 transition-colors duration-200 ml-3 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24" data-v-66c06259><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 6H6a2 2 0 00-2 2v10a2 2 0 002 2h10a2 2 0 002-2v-4M14 4h6m0 0v6m0-6L10 14" data-v-66c06259></path></svg></div></a></div>`);
      });
      _push(`<!--]--></div></div>`);
    } else {
      _push(`<!---->`);
  ...
  _push(`<div class="pr-6 pb-8 w-full" data-v-66c06259><label class="form-label" data-v-66c06259>Request Details:</label>`);
  ...
  _push(`</div><input type="file" accept=".xlsx,.xls,image/*,.doc, .docx,.ppt, .pptx,.txt,.pdf, .zip" class="hidden" multiple="multiple" data-v-66c06259><div class="pr-6 pb-8 w-full lg:w-full flex-col" data-v-66c06259><button type="button" class="btn flex justify-center items-center relative z-10" data-v-66c06259>`);
  _push(ssrRenderComponent(_component_icon, {
    name: "file",
    class: "flex-shrink-0 h-8 fill-gray-400 pr-1"
  }, null, _parent));
  _push(`<h4 data-v-66c06259>${ssrInterpolate(_ctx.$t("Attach files"))}</h4></button>`);
  if ($data.form.files.length) {
    _push(`<!--[-->`);
    ssrRenderList($data.form.files, (file, fi) => {
      _push(`<div class="flex items-center justify-between pr-6 pt-8 w-full lg:w-1/2" data-v-66c06259><div class="flex-1 pr-1" data-v-66c06259>${ssrInterpolate(file.name)} <span class="text-gray-500 text-xs" data-v-66c06259>(${ssrInterpolate($options.getFileSize(file.size))})</span></div><button type="button" class="btn flex justify-center items-center" data-v-66c06259>${ssrInterpolate(_ctx.$t("Remove"))}</button></div>`);
    });
    _push(`<!--]-->`);
  } else {
    _push(`<!---->`);
  }
  _push(`</div></div><div class="px-8 py-4 bg-gray-50 border-t border-gray-100 flex justify-end items-center" data-v-66c06259>`);
...
const Create = /* @__PURE__ */ _export_sfc(_sfc_main, [["ssrRender", _sfc_ssrRender], ["__scopeId", "data-v-66c06259"]]);
export {
  Create as default
};
