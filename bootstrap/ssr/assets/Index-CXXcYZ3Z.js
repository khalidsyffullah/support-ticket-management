...
function _sfc_ssrRender(_ctx, _push, _parent, _attrs, $props, $setup, $data, $options) {
  ...
  _push(`</div>`);
  if ($props.auth.user.role.slug === "customer") {
    _push(`<div class="flex flex-wrap mt-8"><div class="w-full lg:w-1/2 pr-3">`);
    if ($props.customer_tickets.length) {
      _push(`<div class="bg-white rounded-md shadow overflow-x-auto"><table class="w-full whitespace-nowrap"><tr class="text-left font-bold"><th class="pb-4 pt-6 px-6">${ssrInterpolate(_ctx.$t("Ticket Number"))}</th><th class="pb-4 pt-6 px-6">${ssrInterpolate(_ctx.$t("Name"))}</th><th class="pb-4 pt-6 px-6">${ssrInterpolate(_ctx.$t("Status"))}</th></tr><!--[-->`);
      ssrRenderList($props.customer_tickets, (ticket) => {
        _push(`<tr class="hover:bg-gray-100 focus-within:bg-gray-100"><td class="border-t">`);
        _push(ssrRenderComponent(_component_Link, {
          class: "flex items-center px-6 py-4 focus:text-indigo-500",
          href: _ctx.route("tickets.edit", ticket.uid)
        }, {
        ...
        _push(ssrRenderComponent(_component_Link, {
          ...
          default: withCtx((_, _push2, _parent2, _scopeId) => {
            if (_push2) {
              _push2(`${ssrInterpolate(ticket.subject)}`);
            } else {
              return [
                createTextVNode(toDisplayString(ticket.subject), 1)
              ];
            }
          }),
          _: 2
      ...
      _push(`<!--]-->`);
      if ($props.customer_tickets.length === 0) {
        _push(`<tr><td class="px-6 py-4 border-t" colspan="3">${ssrInterpolate(_ctx.$t("No tickets found."))}</td></tr>`);
      } else {
        _push(`<!---->`);
      }
      _push(`</table></div>`);
    ...
    _push(`</div><div class="w-full lg:w-1/2 pl-3">`);
    if ($props.notices.length) {
      _push(`<div class="bg-white rounded-md shadow overflow-hidden"><div class="p-4 border-b"><h2 class="font-bold text-lg">${ssrInterpolate(_ctx.$t("Notice Board"))}</h2></div><ul class="divide-y"><!--[-->`);
      ssrRenderList($props.notices, (notice) => {
        _push(`<li class="p-4"><h3 class="font-bold">${ssrInterpolate(notice.title)}</h3><p class="text-gray-600">${ssrInterpolate(notice.content)}</p></li>`);
      });
      _push(`<!--]--></ul></div>`);
    } else {
      _push(`<!---->`);
  ...
  if ($props.auth.user.role.slug !== "customer") {
    ...
    _push(`</div></div></div>`);
    if ($props.auth.user.role.slug !== "customer" && $props.auth.user.role.slug !== "admin") {
      _push(`<div class="badge__item h-32 w-full cursor-pointer"><div class="l__items bg-white rounded-lg shadow-lg flex justify-between w-full"><div class="badge__info"><span class="title">${ssrInterpolate(_ctx.$t("Lock My Account"))}</span><span class="number"></span></div><div class="a__right">`);
      _push(ssrRenderComponent(_component_icon, {
        name: "lock",
        class: "h-5 fill-gray-400 mr-5 ml-5"
      }, null, _parent));
