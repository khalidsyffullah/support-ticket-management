...
import { resolveComponent, withCtx, createVNode, createBlock, toDisplayString, openBlock, Fragment, renderList, createCommentVNode, createTextVNode, useSSRContext } from "vue";
...
function _sfc_ssrRender(_ctx, _push, _parent, _attrs, $props, $setup, $data, $options) {
  ...
  ssrRenderList($props.users.data, (user) => {
    ...
    _push(ssrRenderComponent(_component_Link, {
      ...
      default: withCtx((_, _push2, _parent2, _scopeId) => {
        if (_push2) {
          _push2(`${ssrInterpolate(user.country ? _ctx.$t(user.country) : "")}`);
        } else {
          return [
            createTextVNode(toDisplayString(user.country ? _ctx.$t(user.country) : ""), 1)
          ];
        }
      }),
      _: 2
    ...
    _push(ssrRenderComponent(_component_Link, {
      ...
      default: withCtx((_, _push2, _parent2, _scopeId) => {
        if (_push2) {
          _push2(`${ssrInterpolate(user.city ? _ctx.$t(user.city) : "")}`);
        } else {
          return [
            createTextVNode(toDisplayString(user.city ? _ctx.$t(user.city) : ""), 1)
          ];
        }
      }),
      _: 2
    ...
    _push(`</td><td class="border-t"><div class="flex items-center px-6 py-4"><span class="${ssrRenderClass({ "text-red-500": user.is_locked, "text-green-500": !user.is_locked })}">${ssrInterpolate(user.is_locked ? _ctx.$t("Locked") : _ctx.$t("Unlocked"))}</span></div></td><td class="w-px border-t">`);
    if (user.role.slug !== "customer") {
      _push(`<button class="${ssrRenderClass([{ "bg-green-500": user.is_locked, "bg-red-500": !user.is_locked }, "flex items-center px-4 py-2 text-sm text-white rounded"])}">${ssrInterpolate(user.is_locked ? _ctx.$t("Unlock") : _ctx.$t("Lock"))}</button>`);
    } else {
      _push(`<!---->`);
    }
  ...
  if ($props.users.data.length === 0) {
    _push(`<tr><td class="px-6 py-4 border-t" colspan="8">No users found.</td></tr>`);
  } else {
    _push(`<!---->`);
  }
