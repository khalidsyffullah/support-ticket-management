...
function _sfc_ssrRender(_ctx, _push, _parent, _attrs, $props, $setup, $data, $options) {
  ...
  if ($props.auth.user.role.slug === "customer") {
    ...
    if ($props.notifications.length) {
      _push(`<div class="bg-white rounded-md shadow overflow-hidden"><div class="p-4 border-b"><h2 class="font-bold text-lg">${ssrInterpolate(_ctx.$t("Notifications"))}</h2></div><ul class="divide-y"><!--[-->`);
      ssrRenderList($props.notifications, (notification) => {
        _push(`<li class="p-4 hover:bg-gray-100 cursor-pointer"><p class="text-gray-600">${ssrInterpolate(notification.data.message)}</p></li>`);
      });
      _push(`<!--]--></ul></div>`);
    } else {
      _push(`<!---->`);
