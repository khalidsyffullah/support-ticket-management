<template>
  <div>
      <div class="flex justify-center w-full flash-message">
          <div v-if="$page.props.flash && $page.props.flash.success && show" ref="successBox" class="mt-3 mb-3 flex items-center justify-between bg-green-500 rounded max-w-3xl">
          ...
          <div v-if="$page.props.flash && ($page.props.flash.error || Object.keys($page.props.errors).length > 0) && show" ref="errorBox" class="mb-8 flex items-center justify-between bg-red-400 rounded max-w-3xl">
              <div class="flex items-center">
...
export default {
  ...
  methods: {
    handleClickOutside(event) {
      ...
      const clickedInside = (successBox && successBox.contains(event.target)) || (errorBox && errorBox.contains(event.target));
